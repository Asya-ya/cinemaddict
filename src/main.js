...
const renderMovies = (start = 0) => {
  for (let i = start; i < Math.min(SHOWN_TASK_COUNT + start, MAIN_TASK_COUNT); i++) {
    renderElement(filmsListContainerElement, new MovieCardView(movies[i]).getElement());
  }
...
const renderTopRatedMovies = (start = 0) => {
  for (let i = start; i < TOP_RATED_TASK_COUNT; i++) {
    renderElement(filmsTopRatedListElement, new MovieCardView(topRatedMovies[i]).getElement());
  }
};

const renderMostCommentedMovies = (start = 0) => {
  for (let i = start; i < MOST_COMMENTED_TASK_COUNT; i++) {
    renderElement(filmsMostCommentedListElement, new MovieCardView(mostCommentedMovies[i]).getElement());
  }
};
...
// renderTemplate(body, createMoviePopupTemplate(movies[0]));
